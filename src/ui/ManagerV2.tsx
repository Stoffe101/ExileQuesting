...
import type { AppSettings, OverlayMode, OverlayTypography, RuntimeState } from '../core/types';
...
function Overview({ state, setState, onNavigate }: { state: RuntimeState; setState: (state: RuntimeState) => void; onNavigate: (tab: AppTab) => void }) {
  ...
  const nextIndex = state.dataset.steps.findIndex((candidate, candidateIndex) => candidateIndex > index && isStepEnabled(candidate, state.settings));
  const next = nextIndex >= 0 ? state.dataset.steps[nextIndex] : undefined;
  return (
...
function applyExperiencePreset(state: RuntimeState, setState: (state: RuntimeState) => void, preset: 'minimal' | 'standard' | 'teach'): void {
...
function Settings({ state, setState }: { state: RuntimeState; setState: (state: RuntimeState) => void }) {
  ...
  return <div className="page custom-scrollbar m2-settings"><div className="page-heading compact-heading"><div><span className="eyebrow">CONFIGURATION</span><h1>Settings</h1><p>Three useful presets first. Fine-grained controls stay in Advanced.</p></div></div>
    <section className="panel m2-presets"><div className="section-title"><h2>Experience preset</h2><span>Recommended: Standard</span></div><div className="m2-preset-grid"><button onClick={() => applyExperiencePreset(state, setState, 'minimal')}><strong>Minimal</strong><span>Experienced players · terse route · compact overlay</span></button><button className="recommended" onClick={() => applyExperiencePreset(state, setState, 'standard')}><strong>Standard</strong><span>Important context · Focus overlay · sane defaults</span></button><button onClick={() => applyExperiencePreset(state, setState, 'teach')}><strong>Teach Me</strong><span>Why, layout help and expanded coaching</span></button></div></section>
    ...
    <details className="panel m2-advanced"><summary>Advanced overlay & accessibility</summary><div className="m2-advanced-grid"><label className="field"><span>Overlay mode</span><select value={state.settings.overlayMode} onChange={(event) => update({ overlayMode: event.target.value as OverlayMode })}><option value="compact">Compact</option><option value="focus">Focus</option><option value="coach">Coach</option></select></label><label className="field"><span>Guidance depth</span><select value={state.settings.guidanceMode} onChange={(event) => update({ guidanceMode: event.target.value as AppSettings['guidanceMode'] })}><option value="racer">Racer</option><option value="balanced">Balanced</option><option value="beginner">Beginner</option></select></label><SettingRow title="Click-through overlay" description={`Toggle interaction with ${state.settings.hotkeys.toggleInteraction}.`}><Toggle checked={state.settings.overlayClickThrough} onChange={(overlayClickThrough) => update({ overlayClickThrough })} /></SettingRow><SettingRow title="Reduced motion" description="Disable non-essential transitions."><Toggle checked={state.settings.reducedMotion} onChange={(reducedMotion) => update({ reducedMotion })} /></SettingRow><SettingRow title="Reduced transparency" description="Use a fully opaque overlay surface."><Toggle checked={state.settings.reducedTransparency} onChange={(reducedTransparency) => update({ reducedTransparency })} /></SettingRow></div></details>
  </div>;
}
