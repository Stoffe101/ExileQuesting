...
export default function CommandPalette({ state, onNavigate }: { state: RuntimeState; onNavigate: (tab: AppTab) => void }) {
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    const base: Command[] = [
      ...
      { id: 'passive', label: 'Passive Plan', detail: passive.title, keywords: 'tree passive next node allocate refund skill point', run: () => onNavigate('guide') },
  ...
  if (!open) return null;
  return (
    <div className="command-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
