...
/**
 * PoE 1 exposes a small static passive-tree UI element near the top-centre of
 * the client. These normalized coordinates mirror Exile-UI's proven PoE 1
 * `skilltree` image-check region instead of trying to recognise passive nodes.
 */
...
function gray(bitmap: Uint8Array, width: number, x: number, y: number): number {
  ...
  // colour channels keeps the signature independent of RGB/BGRA ordering.
