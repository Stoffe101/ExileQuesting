...
for (const target of targets) {
  ...
  const allGemEntries = imported.build.skillStages
    .flatMap((stage) => stage.skillGroups ?? [])
    .flatMap((group) => group.gems)
    .map((gem) => ({ name: gem.name, skillId: gem.skillId }));
  const uniqueGemNames = [...new Set(allGemEntries.map((gem) => gem.name).filter(Boolean))];
  const leakedSupportEntries = allGemEntries
    .filter((gem) => /^Support\s/i.test(gem.name))
    .filter((gem, index, list) => list.findIndex((candidate) => candidate.name === gem.name && candidate.skillId === gem.skillId) === index)
    .slice(0, 30);
  ...
  if (leakedSupportEntries.length) throw new Error(`${target.label}: metadata-derived support gem names leaked after canonicalization: ${JSON.stringify(leakedSupportEntries)}`);
